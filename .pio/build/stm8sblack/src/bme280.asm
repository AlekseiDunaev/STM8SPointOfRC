...
;--------------------------------------------------------
	.area CODE
;	src\bme280.c: 16: void Error(void) {
;	-----------------------------------------
;	 function Error
;	-----------------------------------------
_Error:
;	src\bme280.c: 17: LED_OFF;
	ld	a, #0x20
	ldw	x, #0x5014
;	src\bme280.c: 18: }
	jp	_GPIO_WriteHigh
;	src\bme280.c: 20: void BME280_Setup(void) {
;	-----------------------------------------
;	 function BME280_Setup
;	-----------------------------------------
_BME280_Setup:
;	src\bme280.c: 23: uint8_t res = BME280_ReadReg(BME280_REG_ID);
	ld	a, #0xd0
	call	_BME280_ReadReg
;	src\bme280.c: 28: if (res != BME280_ID) {
	cp	a, #0x60
	jreq	00102$
;	src\bme280.c: 29: Error();
;	src\bme280.c: 30: return;
	jp	_Error
00102$:
;	src\bme280.c: 33: BME280_WriteReg(BME280_REG_SOFTRESET, BME280_SOFTRESET_VALUE);
	push	#0xb6
	ld	a, #0xe0
	call	_BME280_WriteReg
;	src\bme280.c: 34: while (BME280_ReadStatus() & BME280_STATUS_IM_UPDATE);
00103$:
	call	_BME280_ReadStatus
	srl	a
	jrc	00103$
;	src\bme280.c: 36: BME280_ReadCoefficients();
	call	_BME280_ReadCoefficients
;	src\bme280.c: 37: BME280_SetStandby(BME280_STBY_1000);
	ld	a, #0xa0
	call	_BME280_SetStandby
;	src\bme280.c: 38: BME280_SetFilter(BME280_FILTER_4);
	ld	a, #0x08
	call	_BME280_SetFilter
;	src\bme280.c: 39: BME280_SetOversamplingTemper(BME280_OSRS_T_x4);
	ld	a, #0x60
	call	_BME280_SetOversamplingTemper
;	src\bme280.c: 40: BME280_SetOversamplingPressure(BME280_OSRS_P_x2);
	ld	a, #0x08
	call	_BME280_SetOversamplingPressure
;	src\bme280.c: 41: BME280_SetOversamplingHum(BME280_OSRS_H_x1);
	ld	a, #0x01
	call	_BME280_SetOversamplingHum
;	src\bme280.c: 43: value32 = BME280_ReadReg(BME280_REG_CTRL_MEAS);
	ld	a, #0xf4
	call	_BME280_ReadReg
;	src\bme280.c: 44: value32 |= BME280_ReadReg(BME280_REG_CTRL_HUM) << 8;
	ld	a, #0xf2
	call	_BME280_ReadReg
;	src\bme280.c: 57: BME280_SetMode(BME280_MODE_NORMAL);
	ld	a, #0x03
	call	_BME280_SetMode
;	src\bme280.c: 59: LED_ON;
	ld	a, #0x20
	ldw	x, #0x5014
;	src\bme280.c: 60: }
	jp	_GPIO_WriteLow
;	src\bme280.c: 62: void BME280_WriteReg(uint8_t iReg, uint8_t iValue) {
;	-----------------------------------------
;	 function BME280_WriteReg
;	-----------------------------------------
_BME280_WriteReg:
	pushw	x
;	src\bme280.c: 64: iData[0] = iReg;
	ldw	x, sp
	incw	x
	ld	(x), a
;	src\bme280.c: 65: iData[1] = iValue;
	ld	a, (0x05, sp)
	ld	(0x02, sp), a
;	src\bme280.c: 66: I2C_Send_Bytes((BME280_ADDRESS), sizeof(iData), iData);
	pushw	x
	ldw	x, #0x0002
	ld	a, #0xee
